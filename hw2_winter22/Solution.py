...
def createTables():
    ...
    try:
        ...
        conn.execute(
            "CREATE TABLE Matches(id integer UNIQUE NOT NULL, competition TEXT NOT NULL, home_id integer NOT NULL, away_id integer NOT NULL )")
        conn.execute(
            "CREATE TABLE Players(id integer UNIQUE NOT NULL, team_id integer UNIQUE NOT NULL, age integer NOT NULL, height integer NOT NULL, preferred_foot TEXT NOT NULL)")
        conn.execute(
            "CREATE TABLE Stadiums(id integer UNIQUE NOT NULL, capacity integer NOT NULL, belong_to integer UNIQUE)")
...
def addMatch(match: Match) -> ReturnValue:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL(
            "INSERT INTO Matches(id, competition, home_id, away_id) VALUES({id}, {competition}, {home_id}, {away_id})").format(
            id=sql.Literal(id), competition=sql.Literal(competition), home_id=sql.Literal(home_id),
...
def addPlayer(player: Player) -> ReturnValue:
    ...
    if id <= 0 or team_id <= 0 or height <= 0 or (foot not in ['Left', 'Right']):
        return ReturnValue.BAD_PARAMS
    if id is None or team_id is None or age is None or height is None or foot is None:
        return ReturnValue.BAD_PARAMS
...
def addStadium(stadium: Stadium) -> ReturnValue:
    ...
    if id <= 0 or capacity <= 0 or belong_to <= 0:
        return ReturnValue.BAD_PARAMS
    if capacity is None or belong_to is None:
        return ReturnValue.BAD_PARAMS
...
def getStadiumProfile(stadiumID: int) -> Stadium:
    ...
    try:
        conn = Connector.DBConnector()
        query = sql.SQL(
            "SELECT * FROM Stadium WHERE id = {id}").format(
